...
class AES:
    ...
    # Key Expansion function adjusted for AES-256 specifics
    def keyExpansion(self, key, rounds, keySize):
        ...
        while i < 4 * (rounds + 1):
            ...
            elif keySizeInWords == 8 and i % keySizeInWords == 4:  # Additional step for AES-256
                temp = self.substituteBytes(temp)
            roundKeys.append(self.xorWords(roundKeys[i - keySizeInWords], temp))
            i += 1
    ...
    def hamming_distance(self, state1, state2):
        ...
        for s1, s2 in zip(state1, state2):
            # XOR the two states and count the number of 1's (which are the differences)
            differences = bin(int(s1, 16) ^ int(s2, 16)).count('1')
            distance += differences
        return distance

    # the function encrypts the plain text using AES-128/192/256
	# Modified the encrypt function to print the avalanche effect
    def encrypt(self):
        print("[+] Encrypting... ")
        ...
        for j in range(0,len(self.blocks)):
            ...
            prev_state = plaintext  # Keep track of the previous state
            for i in range(1, self.rounds + 1):
                # Perform the round operations
                ciphertext = self.substituteBytes(ciphertext)
                ciphertext = self.shiftRows(ciphertext)
                if i < self.rounds:  # MixColumns is not performed in the final round
                ...
                # Calculate and print the avalanche effect after each round
                distance = self.hamming_distance(prev_state, ciphertext)
                prev_state = ciphertext  # Update the previous state
                print(f"Avalanche effect after round {i}: {distance} bits changed")
